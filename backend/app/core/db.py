...
async def _hydrate_persona_set_row(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "place_key": row.get("place_key"),
        "place_label": row.get("place_label"),
        "place_name": payload_meta.get("place_name") or row.get("place_label"),
        "audience_key": row.get("audience_key") or "",
        "audience_filters": _safe_json(row.get("audience_filters_json"), []),
        "audience_type": payload_meta.get("audience_type") or ", ".join(_safe_json(row.get("audience_filters_json"), [])),
        "scope": row.get("scope") or "shared",
        "shared_asset": bool(row.get("shared_asset")),
        "reusable": bool(row.get("shared_asset")),
        "source_policy": row.get("source_mode") or "open_socials",
        "source_type": payload_meta.get("source_type") or row.get("context_type") or "audience",
        "context_type": row.get("context_type"),
    ...
    return [
        {
            ...
            "place_key": row.get("place_key"),
            "place_label": row.get("place_label"),
            "place_name": (((_safe_json(row.get("payload_json"), {}) or {}).get("meta") or {}).get("place_name") or row.get("place_label")),
            "audience_key": row.get("audience_key") or "",
            "audience_filters": _safe_json(row.get("audience_filters_json"), []),
            "audience_type": (((_safe_json(row.get("payload_json"), {}) or {}).get("meta") or {}).get("audience_type") or ", ".join(_safe_json(row.get("audience_filters_json"), []))),
            "scope": row.get("scope") or "shared",
            "shared_asset": bool(row.get("shared_asset")),
            "reusable": bool(row.get("shared_asset")),
            "source_policy": row.get("source_mode") or "open_socials",
            "source_type": (((_safe_json(row.get("payload_json"), {}) or {}).get("meta") or {}).get("source_type") or row.get("context_type") or "audience"),
            "context_type": row.get("context_type"),
            "source_summary": row.get("source_summary") or "",
            "quality_score": float(row.get("quality_score")) if row.get("quality_score") is not None else None,
            "confidence_score": float(row.get("confidence_score")) if row.get("confidence_score") is not None else None,
