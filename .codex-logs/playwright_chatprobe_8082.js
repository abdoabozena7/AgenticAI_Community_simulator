...
async function getChatState(page, label) {
  const state = await page.evaluate(() => {
    ...
    return {
      ...
      hasClarificationBanner: /clarification|ÊæÖíÍ/i.test(bodyText),
      hasResearchReviewBanner: /review|ãÑÇÌÚÉ|links|ÑæÇÈØ/i.test(bodyText),
...
(async () => {
  ...
  try {
    ...
    await runStep('login-admin', page, async () => {
      ...
      const clicked = await clickByName(page, [/Sign In/i, /ÊÓÌíá ÇáÏÎæá/i]);
    ...
    await runStep('insert-idea-send', page, async () => {
      ...
      await page.fill('[data-testid="chat-input"]', 'ãÊÌÑ ÇÔÊÑÇßÇÊ ÞåæÉ ãÎÊÕÉ ãÏÚæã ÈÇáÐßÇÁ ÇáÇÕØäÇÚí ãÚ ÔÍä åÌíä ÏÇÎá ÇáÎáíÌ');
